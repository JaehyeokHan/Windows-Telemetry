...
# rbs parser for version 1607 rbs files
...
def LogBlockPrint(lbh_buf) :
    ...
    while(True) :
        ...
        deflated_size = strLT2int(lbh_buf[startOffset_logblock+20:startOffset_logblock+24])
        print "blkidx: " + str(block_idx)
        print "endsiz: " + str(encoded_size)
        print "dflsiz: " + str(deflated_size)
        
        print "unknow: " + lbh_buf[startOffset_logblock+24:startOffset_logblock+28].encode("hex")

        startOffset_logblock = startOffset_logblock + 29 + encoded_size + deflated_size
...
def SaveLogBlockInfo(lbh_buf) :
    OutputFileName = "LogBlocks_Info.csv"
    print len(lbh_buf)
    ...
    while(True) :
        nblock += 1
        if (nblock % 100 == 0) : print nblock
        if (lbh_buf[startOffset_logblock] == '\x00' and lbh_buf[startOffset_logblock+1] == '\x00'):
            print "[Break Point #1]"
            break
        
        f.write(str(nblock)+",")
        f.write(lbh_buf[startOffset_logblock+53:startOffset_logblock+65].encode("hex")+",")
        ...
        f.write(lbh_buf[startOffset_logblock+24:startOffset_logblock+28].encode("hex")+",")
...
FilePath = "D:\\1. Research\\55. Windows Telemetry\\_on work\\rbs_file_samples\\1607 ver\\호용선배_rbs\\Events_Normal.rbs"
